...
class SoftmatcherPromptGenerator(BidirectionalPromptGenerator):
    ...
    Args:
        ...
        approximate_matching: bool - Whether to use approximate matching, this will use RFF to approximate
            the similarity map.
        softmatching_score_threshold: float - Threshold for selecting points based on normalized soft correspondence scores.
    ...
    def __call__(
        ...
        for i, target_features in enumerate(target_features_list):
            ...
            for class_id, mask in reference_masks.data.items():
                ...
                # add the soft map to the similarities, this increases the mask filtering technique based on average similarity.
    ...
    def _calculate_similarity(
        ...
        if use_rff:
            ...
            # RFF approximation
            projection_matrix = torch.randn(d, rff_dim // 2, device=ref_features.device) / rff_sigma
