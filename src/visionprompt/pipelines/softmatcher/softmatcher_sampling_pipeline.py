...
class SoftMatcherSampling(SoftMatcher):
    """This is the SoftMatcher sampling pipeline.

    This pipeline uses sampling in the soft matching algorithm to generate prompts for the segmenter.

    Examples:
        >>> from visionprompt.pipelines.softmatcher import SoftMatcherSampling
        ...
        >>>
        >>> # Run learn and infer
        >>> learn_results = soft_matcher_sampling.learn([Image(ref_image)], [ref_priors])
        >>> infer_results = soft_matcher_sampling.infer([Image(target_image)])
        >>>
    ...
    def __init__(
        ...
        """Initialize the SoftMatcher sampling pipeline.
