...
def persam_f(args, obj_name, images_path, masks_path, output_path):
    ...
    for i in tqdm(range(args.train_epoch_outside)):
        ...
        for ref_idx in range(training_size):
            # Path preparation
            ref_image_path = os.path.join(images_path, obj_name, f'{ref_idx:02}.jpg')
            ref_mask_path = os.path.join(masks_path, obj_name, f'{ref_idx:02}.png')
            ...
            # print('======> Mask weights:\n', weights_np)
        print(f'LR: {current_lr:.6f}, Dice_Loss: {dice_loss.item():.4f}, Focal_Loss: {focal_loss.item():.4f}')
