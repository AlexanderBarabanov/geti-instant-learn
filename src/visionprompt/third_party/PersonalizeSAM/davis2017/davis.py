...
class DAVISTestDataset(Dataset):
    def __init__(self, root, imset='2017/val.txt', resolution=480, single_object=False, target_name=None):
        ...
        self.shape = {}
        self.size_480p = {}
        with open(path.join(_imset_f)) as lines:
            for line in lines:
                _video = line.rstrip('\n')
    ...
    def __getitem__(self, index):
        ...
        for f in range(self.num_frames[video]):
            img_file = path.join(self.image_dir, video, f'{f:05d}.jpg')
            img = Image.open(img_file).convert('RGB')
            img = np.array(img, dtype='uint8')
            images.append(img)
            info['frames'].append(f'{f:05d}.jpg')

            mask_file = path.join(self.mask_dir, video, f'{f:05d}.png')
...
class DAVIS:
    ...
    def __init__(self, root, task='unsupervised', subset='val', sequences='all', resolution='480p', codalab=False):
        ...
        if sequences == 'all':
            with open(os.path.join(self.imagesets_path, f'{self.subset}.txt')) as f:
                tmp = f.readlines()
    ...
    def get_frames(self, sequence):
        for img, msk in zip(self.sequences[sequence]['images'], self.sequences[sequence]['masks'], strict=False):
