...
class IMaskDINODecoder(nn.Module):
    ...
    def pred_box(self, reference, hs, ref0=None):
        ...
        for dec_lid, (layer_ref_sig, layer_bbox_embed, layer_hs) in enumerate(zip(reference[:-1], self.bbox_embed, hs, strict=False)):
    ...
    def _set_aux_loss(self, outputs_class=None, outputs_seg_masks=None, out_boxes=None, predictions_iou_score=None, predictions_class_part=None):
        ...
        if out_boxes is None:
            return [
                ...
                for a, b in zip(outputs_class[:-1], outputs_seg_masks[:-1], strict=False)
            ]
        if outputs_seg_masks is None:
            return [
                {"pred_logits": a, "pred_boxes": c}
                for a, c in zip(outputs_class[:-1], out_boxes[:-1], strict=False)
            ]
        if predictions_iou_score is None:
            return [
                {"pred_logits": a, "pred_masks": b, "pred_boxes": c}
                for a, b, c in zip(outputs_class[:-1], outputs_seg_masks[:-1], out_boxes[:-1], strict=False)
            ]
        return [
            {"pred_logits": a, "pred_masks": b, "pred_boxes": c, "pred_ious": d, "pred_logits_part": e}
            for a, b, c, d, e in zip(outputs_class[:-1], outputs_seg_masks[:-1], out_boxes[:-1], predictions_iou_score[:-1], predictions_class_part[:-1], strict=False)
        ]
