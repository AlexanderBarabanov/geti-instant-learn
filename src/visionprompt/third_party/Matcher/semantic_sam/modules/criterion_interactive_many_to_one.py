...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True, key='gt_whole_classes'):
        ...
        idx = self._get_src_permutation_idx(indices)
        target_classes_o = torch.cat([t[key][J] for t, (_, J) in zip(targets, indices, strict=False)])
    ...
    def loss_labels_part(self, outputs, targets, indices, num_boxes, log=True, key='gt_part_classes'):
        ...
        if key not in targets[0].keys() or 'masks' not in targets[0].keys():
            # FIXME only consider batchsize=1 case
            assert len(targets) == 1
    ...
    def loss_boxes_panoptic(self, outputs, targets, indices, num_boxes):
        ...
        src_boxes = outputs['pred_boxes'][idx]
        target_boxes = torch.cat([t['boxes'][i] for t, (_, i) in zip(targets, indices, strict=False)], dim=0)
        target_labels = torch.cat([t['labels'][i] for t, (_, i) in zip(targets, indices, strict=False)], dim=0)
