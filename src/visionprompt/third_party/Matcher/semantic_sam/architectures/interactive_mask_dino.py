...
from ..modules import M2MHungarianMatcher, SetCriterion, SetCriterionOsPartWholeM2M, sem_seg_postprocess
from ..utils import box_ops, configurable, get_iou
    ...
    def from_config(cls, cfg):
        ...
        if deep_supervision:
            ...
            for i in range(dec_layers):
                aux_weight_dict.update({k.replace('_0', f'_{i+1}'): v for k, v in weight_dict.items()})
        ...
        if self.training:
            ...
            if self.task_switch['sam']:
                ...
                for key, value in losses_sam.items():
                    new_losses_sam['sam.'+str(key)] = value
