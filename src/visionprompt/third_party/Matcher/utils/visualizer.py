...
class _PanopticPrediction:
    ...
    def __init__(self, panoptic_seg, segments_info, metadata=None):
        ...
        for sid, area in zip(self._seg_ids, self._seg_areas, strict=False):
...
def _create_text_labels(classes, scores, class_names, is_crowd=None):
    ...
    if scores is not None:
        if labels is None:
            ...
            labels = [f"{l} {s * 100:.0f}%" for l, s in zip(labels, scores, strict=False)]
    if labels is not None and is_crowd is not None:
        labels = [l + ("|crowd" if crowd else "") for l, crowd in zip(labels, is_crowd, strict=False)]
    return labels
...
class Visualizer:
    ...
    def draw_panoptic_seg(self, panoptic_seg, segments_info, area_threshold=None, alpha=0.7):
        ...
        if len(all_instances) == 0:
            return self.output
        masks, sinfo = list(zip(*all_instances, strict=False))
