    ...
    for model_key in model_keys:
        ...
        if model_key in ckpt_keys:
            ...
            if model_weight.shape == ckpt_weight.shape:
                ...
                matched_log.append(f"Loaded {model_key}, Model Shape: {model_weight.shape} <-> Ckpt Shape: {ckpt_weight.shape}")
            else:
                unmatched_log.append(f"*UNMATCHED* {model_key}, Model Shape: {model_weight.shape} <-> Ckpt Shape: {ckpt_weight.shape}")
        else:
            unloaded_log.append(f"*UNLOADED* {model_key}, Model Shape: {model_weight.shape}")
