...
from vis_utils import show_mask, show_neg_points, show_pos_points
...
def get_arguments():
    ...
    parser.add_argument(
        "--ckpt", type=str, default="/segment_anything_model/sam_vit_h.pth",
    )
    ...
    for obj_name in os.listdir(images_path):
        ...
        if ".DS" not in obj_name:
            ...
            for test_idx in range(len(os.listdir(test_images_path))):
                ...
                pred_mask, point_coords, point_labels, dis = p2sam_perseg(
                    args,
                    sam,
                    ref_image_path,
                    ref_mask_path,
                ...
                if args.vis:
                    if test_idx == args.ref_idx:
                        ...
                        gt_output_path = os.path.join(
                            output_path, f"{test_idx}_ref.jpg",
                        )
                        ...
                        pred_output_path = os.path.join(
                            output_path, f"{test_idx}_{dis:.2f}.jpg",
                        )
                        plt.figure(figsize=(10, 10))
                        plt.imshow(test_image)
