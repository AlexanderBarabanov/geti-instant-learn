...
class EfficientViTBackbone(nn.Module):
    def __init__(
        ...
        # stages
        self.stages = []
        for w, d in zip(width_list[1:3], depth_list[1:3], strict=False):
            stage = []
            for i in range(d):
        ...
        for w, d in zip(width_list[3:], depth_list[3:], strict=False):
            stage = []
            block = self.build_local_block(
                in_channels=in_channels,
                out_channels=w,
...
class EfficientViTLargeBackbone(nn.Module):
    def __init__(
        ...
        for stage_id, (w, d) in enumerate(zip(width_list[1:], depth_list[1:], strict=False), start=1):
            stage = []
            block = self.build_local_block(
                block="mb"
