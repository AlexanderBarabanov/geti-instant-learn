...
class NoiseScheduleVP:
    def __init__(
        ...
        if schedule not in ["linear", "discrete", "cosine"]:
            raise ValueError(
                f"Unsupported noise schedule {schedule}. The schedule needs to be 'linear' or 'cosine'",
            )
    ...
    def dpm_solver_third_update(
        ...
        if self.predict_x0:
            ...
            if solver_type == "dpm_solver":
                ...
                raise ValueError(
                    f"solver_type must be either dpm_solver or dpm_solver++, got {solver_type}",
                )
        else:
            phi_11 = torch.expm1(r1 * h)
            phi_12 = torch.expm1(r2 * h)
