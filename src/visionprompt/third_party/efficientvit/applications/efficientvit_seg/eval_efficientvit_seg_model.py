...
class Resize:
    def __init__(
        ...
        interpolation: int | None = cv2.INTER_CUBIC,
    ):
...
class CityscapesDataset(Dataset):
    ...
    def __init__(self, data_dir: str, crop_size: tuple[int, int] | None = None):
        super().__init__()
                ...
                if args.save_path is not None:
                    with open(os.path.join(args.save_path, "summary.txt"), "a") as fout:
                        for i, (idx, image_path) in enumerate(
                            zip(feed_dict["index"], feed_dict["image_path"], strict=False),
                        ):
