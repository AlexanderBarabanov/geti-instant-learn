...
example_pairs = [
    {"label": "Bag Example", "reference": "data/PerSeg/Images/backpack/00.jpg", "target": "data/PerSeg/Images/backpack/01.jpg"},
    {"label": "Peanuts Example", "reference": "data/peanuts_coco/train/WIN_20220423_18_13_48_Pro_jpg.rf.2f7f31f6c6e102cab343008ab4f45b6f.jpg", "target": "data/peanuts_coco/train/WIN_20220502_18_31_01_Pro_jpg.rf.ef0644a70513801a980796f92d6046b1.jpg"},
    {"label": "Potatoes1", "reference": "data/Potatoes/1 1.bmp", "target": "data/Potatoes/2 1.bmp"},
    {"label": "Potatoes2", "reference": "data/Potatoes/1 1.bmp", "target": "data/Potatoes/8 1.bmp"},
    {"label": "Potatoes Large Scene", "reference": "data/Potatoes/scene00001.jpg", "target": "data/Potatoes/scene00121.jpg"},
]
...
zsl_sam_prompter: SAMLearnableVisualPrompter | SAMPartAwareLearnableVisualPrompter | PerSamPredictor | None = None
...
def get_sam_output(point_prompts: List[Prompt], reference_img: np.array, target_img: np.array) -> tuple[np.array, tuple[np.array, str], List]:
    input_image = cv2.imread("data/input_img.jpg")
    reference_features, masks = zsl_sam_prompter.learn(reference_img, points=point_prompts)
    ...
    print("shape of reference features after learning:", reference_features.feature_vectors.shape)
    ...
    for i, prompt in enumerate(point_prompts):
        cv2.circle(input_image, prompt.data, int(input_image.shape[0] / 100), point_colors[prompt.label] , -1)
...
def on_select(evt: gr.SelectData):
    ...
    for prompt in clicks:
        cv2.circle(img_with_inputs, prompt.data, int(original_img.shape[0] / 100), point_colors[prompt.label], -1)
...
def process_predictions():
    ...
    if clicks:
        input_img_with_mask, (result_img, caption), gallery_list = get_sam_output(clicks, original_img, target_image)
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        ...
        model_selector = gr.Radio(
            ...
            choices=["MobileSAM (MAPI)", "MobileSAM", "SAM", "EfficientViT-SAM"],
            value=model_name,
        )
        model_selector.change(init_model, inputs=model_selector)
        label_slider = gr.Slider(
            minimum=0, maximum=1, step=1, value=0, label="Select Label", interactive=True,
        )
        label_slider.change(lambda val: set_label_mode(val), inputs=label_slider)

    with gr.Row():
        reference_image = gr.Image(type="numpy", label="Upload Image", interactive=True, value=default_reference_image)
        target_image = gr.Image(type="numpy", label="Upload Target Image", interactive=True, value=default_target_image)
    ...
    submit_button.click(process_predictions, outputs=[reference_image, target_image, gallery])
