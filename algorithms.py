    ...
    def learn(self, image: np.array, boxes: list[Prompt] | None = None,
        ...
        for class_idx, mask in mask_per_class.items():
            ...
            if isinstance(self.model, SamPredictor):
                ...
                reference_mask = F.interpolate(reference_mask.float(), size=image_embedding.shape[0: 2], mode="bilinear")
                reference_mask = reference_mask.squeeze()[0] # 64 64
        ...
        return VisualPromptingFeatures(
            feature_vectors=np.stack([v.cpu().numpy() for v in self.reference_features.values()]),
            used_indices=np.array(self.reference_features.keys()),
        ), np.stack(list(self.reference_masks.values()))

    def infer(self, target_image, reference_features=None, apply_masks_refinement: bool = True,
              dev: bool = False) -> ZSLVisualPromptingResult:
            ...
            for i, (x, y, score) in enumerate(point_prompt_candidates):
                ...
                if isinstance(self.model, SamPredictor):
                    masks, scores, low_res_logits, *_ = self.model.predict(
                        ...
                        target_embedding=self.reference_embedding[class_idx]  # Target-semantic Prompting (not in model api)
                    )
                elif isinstance(self.model, EfficientViTSamPredictor):
                ...
                if not apply_masks_refinement:
                    ...
                    final_mask, masks, scores = self.post_refinement(low_res_logits, point_coords, point_labels)
        ...
        for label in final_point_prompts:
            prediction[label] = PredictedMask(mask=all_masks[label], points=final_point_prompts[label],
...
def run_per_segment_anything(model: SamPredictor, sample: Series, output_root: str, post_refinement=True, show=False,
                             save=False) -> tuple[plt.Figure, float, float, float]:
    ...
    sim = model.model.postprocess_masks(
        ...
        original_size=model.original_size).squeeze()

    # Positive-negative location prior
    topk_xy_i, topk_label_i, last_xy_i, last_label_i = PersonalizeSAM.persam.point_selection(sim, topk=1)
    ...
    if save:
        ...
        plt.title(f"Mask {best_idx}, iou {sample_iou:.2f}, acc {sample_accuracy:.2f}", fontsize=18)
        plt.axis('off')
        if save:
            filename_without_ext = sample.file_name.split('.')[0]
            vis_mask_output_path = os.path.join(output_path, f'vis_mask_{filename_without_ext}.jpg')
...
def run_p2sam(model: SamPredictor, sample: Series, output_root: str, post_refinement=True, show=False, save=False) -> \
        tuple[plt.Figure, float, float, float]:
    pass


def finetune_persam_F(predictor: SamPredictor, images_path, annotations_path, output_path) -> SamPredictor:
    pass
