...
class Trainer(Evaluator):
    ...
    def resume_from_checkpoint(self, checkpoint_path: str):
        ...
        self.print_and_f_log("optimizer loaded\n")
        if self.enable_amp:
            self.scaler.load_state_dict(checkpoint["scaler"])
            self.print_and_f_log("scaler loaded\n")
        if self.cfg.resume_schedule:
            ...
            self.print_and_f_log("lr scheduler loaded\n")
            if get_dist_size() == checkpoint["train_generator_state"].shape[0]:
                ...
                self.print_and_f_log("train generator state loaded\n")
                torch.set_rng_state(checkpoint["torch_rng_state"][self.rank])
                self.print_and_f_log("torch rng state loaded\n")
                torch.cuda.set_rng_state(checkpoint["torch_cuda_rng_state"][self.rank])
                self.print_and_f_log("torch cuda rng state loaded\n")
            else:
                self.print_and_f_log("warning: failed to load rng states\n")
            if self.cfg.compute_fid:
        ...
        with tqdm(
            ...
            for _, (images, labels) in enumerate(self.train_data_provider.train):
                ...
                if np.isnan(mean_loss):
                    ...
                    self.print_and_f_log("NaN detected, break from training")
                    break
    ...
    def train(self) -> None:
        for epoch in itertools.count(start=self.start_epoch):
            ...
            if self.NaN_detected:
                self.print_and_f_log("NaN detected, breaking from train")
                break
