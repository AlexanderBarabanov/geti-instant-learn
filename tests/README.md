...
This directory contains the testing framework for the `visionprompt` library. We use `pytest` for test discovery and execution.

## Directory Structure

The `tests` directory mirrors the `src/visionprompt` package structure. This makes it easy to locate the tests for a specific module. For example, tests for a module at `src/visionprompt/filters/masks/example.py` should be placed in `tests/filters/masks/test_example.py`.
