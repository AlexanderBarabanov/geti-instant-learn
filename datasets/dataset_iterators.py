...
class BatchedSingleCategoryIter(DatasetIter):
    ...
    def __getitem__(self, batch_index: int) -> Tuple[List[np.ndarray], List[np.ndarray]]:
        images = self._parent.get_images_by_category(self._category_index, start=self._batch_index * self._batch_size, end=(batch_index+1) * self._batch_size)
        masks = self._parent.get_masks_by_category(self._category_index, start=self._batch_index * self._batch_size, end=(batch_index+1) * self._batch_size)
        return images, masks
    ...
    def __len__(self):
        return math.ceil(self._parent.get_image_count_per_category(self._category_index) / self._batch_size)
    ...
    def get_image_filename(self, *indices: int):
        return self._parent.get_image_filename_in_category(self._category_index, indices[0] * self._batch_size + indices[1])
...
class BatchedCategoryIter(DatasetIter):
    ...
    def __getitem__(self, category_index: int) -> BatchedSingleCategoryIter:
        return BatchedSingleCategoryIter(self._parent, self._batch_size, self._category_index)
    ...
    def get_image_filename(self, *indices: int):
        return self._parent.get_image_filename_in_category(indices[0], indices[1] * self._batch_size + indices[2])
